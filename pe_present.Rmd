...
::: {.center data-latex=""}
Under RCT we are guaranteed positivity and exchangeability, and we KNOW one model is right.
:::
...
In this analysis we are approaching categorical and continuous outcome--we have not pursued survival analysis,yet.
  ...
  - Use the g-formula as out outcome model; others are potential candidates too
...
## Confidence Intervals
  ...
  - Bootstrapping to obtain a standard error; also used closed form solution [^3]


Also a closed form solution available:
$$
\widehat{\mathbb{V}}\left(\widehat{\mu}^{}\right)=\frac{1}{n^{2}} \sum_{i=1}^{n}
\left[\frac{R_{i} Y_{i}}{\pi\left(\mathbf{X}_{i} ; \widehat{\boldsymbol{\alpha}}\right)}+\left\{1-\frac{R_{i}}{\pi\left(\mathbf{X}_{i} ; \widehat{\boldsymbol{\alpha}}\right)}\right\} m\left(\mathbf{X}_{i} ; \widehat{\boldsymbol{\beta}}\right)-\widehat{\mu}^{}\right]^{2}
$$

where $\widehat{\mu}=$ AIPW estimate
\vspace{0.3cm}
...
- Tighter confidence intervals from lower SE around inbiased estimates
...
RCTs are typically designed with power in mind to make sure that the correct number of people are enrolled to answer a question without enrolling too many people as to make the trial unethical.
  ...
  - Power typically set at 90% (or higher)
...
Simulation seems to be the best solution so far [^4], which requires many assumptions about the covariance structure of the data
